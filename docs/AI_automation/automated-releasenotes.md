...
| Release-notes file | `docs/internal_release_notes.md` | Displays the current autogenerated section and archived weekly release notes. |

## Workflow Triggers

The **Release Notes Cycle** workflow runs in the following conditions:

- **Merged PR:** When a PR targeting `main` is closed and merged. Closed PR(s) that are not merged are ignored.
...
The script reads the PR number, title, author, merge date, and URL. It then adds a release notes under **Autogenerated release notes** in `docs/internal_release_notes.md`.
